	...
	namespace Native {
		...
		public static class Function {
			...
			private static unsafe object GetResult(Type type, ulong* o) {
				// Log($"GetResult() reading {type.Name} from {new IntPtr(o).ToInt64()}");
		...
		public unsafe static class MemoryAccess {
			...
			*/
			...
			public static String ReadString(IntPtr addr, int off) {
				if( addr != IntPtr.Zero ) {
					unsafe {
						byte* start = (byte*)addr.ToPointer();
						int len = 0;
						while( start[len] != 0 ) {
							++len;
						}
						if( len > 0 ) {
							return UTF8.GetString((byte*)addr.ToPointer(), len);
						}
					}
				}
				return string.Empty;
			...
			public static bool IsBitSet(IntPtr addr, int off, uint bit) {
				if( addr != IntPtr.Zero && bit < 32 ) {
					int mask = 1 << (int)bit;
					unsafe {
						return (*(int*)addr.ToPointer() & mask) != 0;
					}
				}
				return false;
			}
